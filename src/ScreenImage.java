...
public class ScreenImage
{
     ...
     *  @exception IOException if an error occurs during writing
     */
    public static void writeImage(BufferedImage image, File directory, String fileName)
        throws IOException
    {
        if (fileName == null){
            String message = "file name was now specified";
        ...
        if (Arrays.asList( ImageIO.getWriterFileSuffixes() ).contains(type))
        {
            ...
            ImageIO.write(image, "png", outputFile);//save an image of the draw panel
        }
        else
        {
